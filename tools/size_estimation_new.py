...
def setup_config():
    model_path = "/home/hoangphuc/DenseFusion/MASK_RCNN_TOMO/trained_models/model_final_resnet101_fpn.pth"
    cfg = get_cfg()
...
def main():
    ...
    try:
        ...
        while True:
            ...
            label = segmentation['instances'].pred_masks.cpu().numpy()
            label = np.sum(label, axis=0).astype(np.uint8)
            ...
            if len(bboxes) > 0:
                ...
                for i in range(0,len(bboxes)):
                    ...
                    class_id = segmentation['instances'].pred_classes[i]
                    idx = class_id - 1
