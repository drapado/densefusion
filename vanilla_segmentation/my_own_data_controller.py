...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug 27 11:03:02 2019
...
class SegDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        label_file = self.path[index].split(' ')[1].rstrip('\n') + ".png"
        rgb_file = self.path[index].split(' ')[0] + ".jpg"
        #label = np.array(Image.open('{0}/{1}-label.png'.format(self.root, self.path[index])))
        label = np.array(Image.open(self.root + "/" + label_file))
        ...
        if not self.use_noise:
            ...
            rgb = np.array(Image.open(self.root + "/" + rgb_file))
        else:
            #rgb = np.array(self.trancolor(Image.open('{0}/{1}-color.png'.format(self.root, self.path[index])).convert("RGB")))
            rgb = np.array(self.trancolor(Image.open(self.root + "/" + rgb_file)))
...
if __name__ == "__main__":
    root = "/home/hoangphuc/DenseFusion/datasets/my_own_dataset"
    txtlist = "/home/hoangphuc/DenseFusion/datasets/my_own_dataset/train_data_list.txt"
    dataset = SegDataset(root, txtlist, True, 265)
