...
if __name__ == '__main__':
    ...
    dataset = SegDataset(opt.dataset_root, '../datasets/my_own_dataset/train_data_list.txt', True, 688)
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=int(opt.batch_size), shuffle=True, num_workers=int(opt.workers))
    test_dataset = SegDataset(opt.dataset_root, '../datasets/my_own_dataset/test_data_list.txt', False, 307)
    ...
    for epoch in range(1, opt.n_epochs):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            if train_time != 0 and train_time % 1000 == 0:
                torch.save(model.state_dict(), os.path.join(opt.model_save_path, 'model_current_{}.pth'))
            train_time += 1
