...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug 13 13:53:01 2019
...
class TestImageData(data.Dataset):
    def __init__(self, num, add_noise, root, noise_trans, refine):
        ...
        self.rgb_img = self.root + "/0000.png"
        self.label_img = self.root + "/0000_label.png"
        self.depth_img = self.root + "/0000_depth.png"
        
       
        

        #meta_file = open('{0}/data/{1}/gt.yml'.format(self.root, '%02d' % item), 'r')
        meta_file = open('{0}/gt.yml'.format(self.root), 'r')
        self.meta = yaml.load(meta_file)
        self.pt = ply_vtx('{0}/obj_01.ply'.format(self.root))
            
        #print("Object {0} buffer loaded".format(item))

        self.length = len(self.rgb_img)
    ...
    def get_item(self):
        img = Image.open(self.rgb_img)
        img = np.array(img)
        depth = np.array(Image.open(self.depth_img))
        label = np.array(Image.open(self.label_img))
        ...
        return torch.from_numpy(cloud.astype(np.float32)).unsqueeze(0), \
               ...
               self.norm(torch.from_numpy(img_masked.astype(np.float32))).unsqueeze(0), \
               torch.from_numpy(target.astype(np.float32)).unsqueeze(0), \
               torch.from_numpy(model_points.astype(np.float32)).unsqueeze(0),\
               torch.LongTensor([self.objlist.index(1)]).unsqueeze(0)
